...
let subConfig =
	'https://raw.githubusercontent.com/3094069214/Clash/refs/heads/main/ACL4SSR_Online_Full_Mannix_No_DNS_Leak.ini';
...
async function handleTCPOutBound(
	...
	async function connectAndWrite(address, port, socks = false) {
		...
		//if (/^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?).){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/.test(address)) address = `${'www.'}${address}${'.ip.090227.xyz'}`;
	...
	async function retry() {
		if (enableSocks) {
			...
			if (!proxyIP || proxyIP == '') {
				proxyIP = 'PROXYIP.tp1.090227.xyz';
...
function socks5AddressParser(address) {
	...
	//if (/^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?).){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/.test(hostname)) hostname = `${'www.'}${hostname}${'.ip.090227.xyz'}`;
	// 返回解析后的结果
	return {
...
const 啥啥啥_写的这是啥啊 = 'dmxlc3M='; // 替换 atob('ZG14bGMzTT0=')
function 配置信息(UUID, 域名地址) {
	const 协议类型 = 'vless'; // 替换 atob(啥啥啥_写的这是啥啊)
	...
	const 威图瑞 =
		...
		'p' +
		`${
			'tion=' + 加密方式 // 替换 atob('dGlvbj0=') + 加密方式
		}\u0026\u0073\u0065\u0063\u0075\u0072\u0069\u0074\u0079\u003d${
			传输层安全[0]
...
const cmad = decodeURIComponent(
	"telegram 交流群 请勿商用~国外线路!<br>\n<a href='https://t.me/CMLiussss'>https://t.me/CMLiussss</a><br>\n------------------------------------------------------<br>\ngithub 项目地址 Star!Star!Star!!!<br>\n<a href='https://github.com/cmliu/edgetunnel'>https://github.com/cmliu/edgetunnel</a><br>\n------------------------------------------------------<br>\n#####################################################################",
);
/**
 * @param {string} userID
...
async function 生成配置信息(
	...
	if (
		...
		if (go2Socks5s.length > 0 && enableSocks) {
			...
			if (go2Socks5s.includes('all in') || go2Socks5s.includes('*'))
				socks5List += `${decodeURIComponent(
		...
		if (sub) {
			if (enableSocks)
				订阅器 += `CFCDN（访问方式）: Socks5<br>&nbsp;&nbsp;${newSocks5s.join(
					'<br>&nbsp;&nbsp;',
				)}<br>${socks5List}`;
			else if (proxyIP && proxyIP != '')
				订阅器 += `CFCDN（访问方式）: ProxyIP<br>&nbsp;&nbsp;${proxyIPs.join(
					'<br>&nbsp;&nbsp;',
				)}<br>`;
			...
			let 判断是否绑定KV空间 = '';
			if (env.KV)
				判断是否绑定KV空间 = ` <a href='${_url.pathname}/edit'>编辑优选列表</a>`;
		...
		if (动态UUID && _url.pathname !== `/${动态UUID}`) 订阅器 = '';
			...
			订阅器 += `<br>SUBAPI（订阅转换后端）: ${subProtocol}://${subConverter}<br>SUBCONFIG（订阅转换配置文件）: ${subConfig}`;
		const 动态UUID信息 =
			uuid != userID
		...
		let url = `${subProtocol}://${sub}/sub?host=${fakeHostName}&uuid=${
			fakeUserID + 'JmVkZ2V0dW5uZWw9Y21saXUmcHJveHlpcD0=' + RproxyIP
		...
		if (
			...
			if (
				...
				(_url.searchParams.has('loon') && !userAgent.includes('subconverter'))
			) {
		...
		try {
			...
			if ((!sub || sub == '') && isBase64 == true) {
				...
				const response = await fetch(url, {
					headers: {
						'User-Agent': UA + ' CF-Workers-edgetunnel/cmliu', // 替换 atob('IENGLVdvcmtlcnMtZWRnZXR1bm5lbC9jbWxpdQ==')
					},
				});
			...
			api.map(apiUrl =>
				fetch(apiUrl, {
					...
					headers: {
						...
						'User-Agent': 'CF-Workers-edgetunnel/cmliu', // 替换 atob('Q0YtV29ya2Vycy1lZGdldHVubmVsL2NtbGl1')
					},
...
function 生成本地订阅(
	...
	if (noTLS == 'true') {
		...
		notlsresponseBody = uniqueAddressesnotls
			.map(address => {
				...
				const 维列斯Link = `${协议类型}://${UUID}@${address}:${
					port + '?encryption=none&security=&type=ws&host=' // 替换 atob('P2VuY3J5cHRpb249bm9uZSZzZWN1cml0eT0mdHlwZT13cyZob3N0PQ==')
				}${伪装域名}&path=${encodeURIComponent(最终路径)}#${encodeURIComponent(
					addressid + 节点备注,
				)}`;
	...
	const responseBody = uniqueAddresses
		.map(address => {
			...
			const 协议类型 = 'vless'; // 替换 atob(啥啥啥_写的这是啥啊)
			const 维列斯Link = `${协议类型}://${UUID}@${address}:${
				port + '?encryption=none&security=tls&sni=' // 替换 atob('P2VuY3J5cHRpb249bm9uZSZzZWN1cml0eT10bHMmc25pPQ==')
			}${伪装域名}&fp=random&type=ws&host=${伪装域名}&path=${encodeURIComponent(
				最终路径,
		...
		const html = `
			...
			<body>
				...
				<div id="noticeContent" class="notice-content">
					1. ADDAPI 如果是纯IP，可以附带PROXYIP的选项，可将内容添加到proxyip池子中，例如：<br>
					&nbsp;&nbsp;https://raw.githubusercontent.com/cmliu/WorkerVless2sub/main/addressesapi.txt<strong>?proxyip=true</strong><br><br>
					2. ADDAPI 如果是 <a href='https://github.com/XIU2/CloudflareSpeedTest'>CloudflareSpeedTest</a> 的 csv 测速文件，例如：<br>
					&nbsp;&nbsp;https://raw.githubusercontent.com/cmliu/WorkerVless2sub/main/CloudflareSpeedTest.csv<br><br>
					&nbsp;&nbsp;- 如需指定2053端口可以"?port=2053"参数添加到链接末尾，例如：<br>
					&nbsp;&nbsp;https://raw.githubusercontent.com/cmliu/WorkerVless2sub/main/CloudflareSpeedTest.csv<strong>?port=2053</strong><br><br>
					&nbsp;&nbsp;- 如需指定节点备注可以"?id=CF优选"参数添加到链接末尾，例如：<br>
					&nbsp;&nbsp;https://raw.githubusercontent.com/cmliu/WorkerVless2sub/main/CloudflareSpeedTest.csv<strong>?id=CF优选</strong><br><br>
					&nbsp;&nbsp;- 如需指定多个参数则需要使用'&'做间隔，例如：<br>
					&nbsp;&nbsp;https://raw.githubusercontent.com/cmliu/WorkerVless2sub/main/CloudflareSpeedTest.csv?id=CF优选<strong>&</strong>port=2053<br>
				</div>
